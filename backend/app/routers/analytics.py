"""Аналитика: приём событий от mobile + отчёты для клиник и аптек."""
...
async def track_events(
    ...
    """Приём батча событий. Авторизация опциональна."""
...
async def get_my_clinic_analytics(
    ...
    """Отчёт для владельца клиники. Доступно всем клиникам."""
    r = await db.execute(
...
async def get_branch_analytics(
    ...
    """Отчёт для филиала аптеки. Доступно всем аптекам-владельцам."""
    r = await db.execute(
